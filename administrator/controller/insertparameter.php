...
for($i = 0; $i < sizeof($param); $i++){
  ...
  if($resultCheckduplicate){
    $strSQL = sprintf("UPDATE ".$tbprefix."parameter SET par_status = '%s' WHERE par_id = '%s'", mysql_real_escape_string($_POST['to']), mysql_real_escape_string($resultCheckduplicate[0]['par_id']));
    $resultUpdate = $db->update($strSQL);
  }else{
...
if($check==(sizeof($param))){
  print "Y";
}else{
  print "N";
  // print $check." ".sizeof($param);
}
