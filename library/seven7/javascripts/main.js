    ...
    $("#wizard").bootstrapWizard({
      ...
      onNext: function(tab, navigation, index) {
        ...
        if (index === 1) {
          ...
          if (!$("#admin_pwd").val()) {
            $("#admin_pwd").focus();
            $("#admin_pwd").addClass("has-error");
            check++;
          }else{
            $("#admin_pwd").removeClass("has-error");
            $adminpwd = $("#admin_pwd").val();
          }

          if (!$("#admin_pwd2").val()) {
            $("#admin_pwd2").focus();
            $("#admin_pwd2").addClass("has-error");
            check++;
          }else{
            $("#admin_pwd2").removeClass("has-error");
          }
        ...
        if($current==2){
          $('#spnsitename').text($sitename);
          $('#spndesc').text($description);
          $('#spnadmusername').text($adminname);

          if($adminpwd!=''){
            var $len = $adminpwd.length;
            var $pass = createPassword($len);
            $('#spnadmpwd').text($pass);
          }

          $('#spnhostname').text($host);
          $('#spnusername').text($user);

          $('#spndbname').text($dbname);
          $('#spntbname').text($tableprefix);
